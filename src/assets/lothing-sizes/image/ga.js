$(function () {
    ...
    $(".size_left h3:first span").css({"color": "#fff", "background": "url(image/baidu_select_bg2.gif) no-repeat"});
...
//alert(idnum);
            ...
            $(".size_left h3 span").css({"color": "#2f2e2e", "background": "url(image/baidu_select_bg1.gif) no-repeat"});
            $(".size_left ul li").css({"color": "#2f2e2e", "background-color": "#fff"});
            $(this).css({"color": "#fff", "background": "url(image/baidu_select_bg2.gif) no-repeat"});
            $("input[type='text']").val("");
        })
    })
...
//1-S,2-M,3-L,4-XS,5-XL,6-XXL
    ...
    for (var i = 0; i < sarr.length / 5; i++) {
        ...
        if (i == sarr.length / 5 - 1) {
            alert("亲！你的身材实在是太棒了，已经超出了普通人的范畴！");
        }
    }
    if (ci == 6) { //女装裤子
        switch (size) {
